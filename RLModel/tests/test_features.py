...
def test_zscore_clipped_to_three(sample_df):
    ...
    """
    df = compute_features(sample_df.copy())
    z_cols = ["macd_line", "macd_signal", "atr_14", "volume_change"]
...
def test_feature_columns_present(sample_df):
    """All 9 expected market feature columns must exist in output."""
    df = compute_features(sample_df.copy())
    required = ["returns_1h", "rsi_14", "macd_line", "macd_signal",
